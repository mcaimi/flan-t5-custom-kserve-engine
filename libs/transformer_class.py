    ...
    async def predict(
        ...
        if isinstance(payload, Dict):  # v1 api call
            ...
            try:
                ...
                output = InferOutput(
                    name="result", shape=[1], datatype="BYTES", data=[f"{e}"]
                )
                return InferResponse(
                    response_id=req_uuid, model_name=self.name, infer_outputs=[output]
                )
